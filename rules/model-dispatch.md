...
- **Fable (dispatched top model)** — narrow-width, always justified: judge-panel candidates on pivotal designs, the synthesis/final-ruling stage of a workflow (exactly one, at the end), deep design assessments, and the highest-stakes critic lens in an adversarial run — the slot where an uncaught error is irrecoverable. Know what it buys: a Fable subagent is the SAME model as a top-tier orchestrator — fresh context and no sunk-cost attachment to the artifact, not extra capability. Mind tier propagation: adversarial's "critic ≥ generator" rule means a Fable GENERATOR forces Fable critics at 3× width — default generators to Opus and spend Fable on the critic lens that guards the irrecoverable property.
- **Inline (top model, no dispatch)** — only when the work is high-risk AND under-specified: the spec would take longer to write than the change, correctness depends on judgment calls that can't be delegated, or the change touches auth/data-integrity code where a review pass isn't enough. Inline vs dispatched-Fable: inline when the judgment needs the conversation's full context; dispatch when fresh eyes are the point.
